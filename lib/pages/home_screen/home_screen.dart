...
import 'package:mus_greet/core/widgets/appdrawer_widget.dart';
...
import 'package:mus_greet/pages/Notification_Screen/notification_screen.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  // String timeAgoSinceDate(TemporalDateTime date, {bool numericDates = true}) {
  ...
  commentsCountPostCardWidget(Post postData, User UserData, int index, User sessionUser) {
    print(postData);
    print(UserData);
        ...
        switch (snapshot.connectionState) {
          case ConnectionState.done:
            ...
            return PostCardWidget(
              ...
              //timeAgo: timeAgoSinceDate(postData.created_date),
              //image: ImageConstants.IMG_POST1,
              post: postData.post,
  ...
  _loadCommentScreen(Post PostObject, User UserObject, String CommentsCount, User sessionUser) {
    ...
    Navigator.push(context,
        MaterialPageRoute(
          builder: (context) => NotificationScreen()
              //FriendSearch(sessionUser:sessionUser),
        )
    );
  ...
  Future<List<Post>> queryPosts() async {
    try {
      //List<Posts>
      Postss = await Amplify.DataStore.query(Post.classType,where:Post.MOSQUE_ID.eq(""));
      print("inside posts");
      print(Postss);
      return Postss;
    } catch (e) {
  ...
  Future<void> _getLikesCount(String PostID) async {
    LikesCount = 0;
    try {
      List<PostLike> PostLikess = await Amplify.DataStore.query(PostLike.classType, where: PostComment.POST_ID.eq(PostID));
    } catch (e) {
      print("Could not query DataStore: " + e);
    }
  ...
  _getPostCardWidgetUI(Post postData,User UserData, int index, int CommentsCount,){
    ...
    if(index == Postss.length-1) {
      ...
      return PostCardWidget(
        ...
        isSponsored: false,
        //timeAgo: AppTexts.TEMP_TIME_AGO_1,
        timeAgo: 'E12 6PL', // UserData.postcode,
        //image: ImageConstants.IMG_POST1,
        post: postData.post,
        image: postData.post_image_path,
        //image: "https://musgreetphase1images184452-staging.s3.eu-west-2.amazonaws.com/public/image_picker5824495182282881133.jpg",
