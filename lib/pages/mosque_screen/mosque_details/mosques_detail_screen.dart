...
class _MosquesDetailsScreenState extends State<MosquesDetailsScreen>
  ...
  Widget build(BuildContext context) {
    ...
    //mosqueAdmin = true;
    if(mosqueAdmin == true){
      print("inside mosque admin is true");
      return FutureBuilder<List<Mosque>>(
        future: getMosqueForAdmin(),
        builder: (ctx, snapshot) {
          switch (snapshot.connectionState) {
            case ConnectionState.done:
              adminMosques= snapshot.data;
                return buildUI(adminMosques,"admin");
            default:
  ...
  _getBody(String userType) {
    print("inside the body of main");
    print(adminMosques);
    if(userType =="admin"){
      print("inside user type is admin");
      return SingleChildScrollView(
        child: adminMosques.isEmpty ? Container(
            child: Column(
              children: [
                Stack(
                  children: <Widget>[
                ...
                Container(
                  ...
                  child: Container(
                    ...
                    padding: const EdgeInsets.only(left: 80.0, top: 90, right: 80.0,bottom: 360.0),
                    child: ActionButtonWidget(
                      callBack: () {
                        Navigator.of(context)
                            .push(MaterialPageRoute(builder: (context) => CreateMosqueScreen(sessionUser:widget.sessionUser,isEdit: false,)));
                      },
                      text: "Create Mosque",
                      isFilled: true,
                    ),
                ...
                //     children: [
                //       _getMosqueDetails(adminMosques),
                //       CustomSpacerWidget(
                //         width: 2,
                //       ),
                //       Flexible(
        ...
        Column(
          children: [
            _getUpperSection(adminMosques),
            _getTabBar(),
            Center(
              child: [
                HomeTab(mosque: adminMosques,sessionUser: widget.sessionUser,),
                ContactTab(mosque: adminMosques),
                MosqueAboutTab(mosque: adminMosques,sessionUser: widget.sessionUser),
                FacilitiesTab(mosque: adminMosques,sessionUser: widget.sessionUser),
      ...
      return SingleChildScrollView(
        child: Column(
          children: [
            _getUpperSection(mosque),
            _getTabBar(),
            Center(
              child: [
                HomeTab(mosque: mosque,sessionUser: widget.sessionUser,),
                ContactTab(mosque: mosque),
                MosqueAboutTab(mosque: mosque,sessionUser: widget.sessionUser),
                FacilitiesTab(mosque: mosque,sessionUser: widget.sessionUser),
              ][_tabController.index],
            )
          ],
        ),
  ...
  _getUpperSection(List<Mosque> Mosques) {
    return Container(
      child: Column(
        children: [
          Stack(
          ...
          Container(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                _getMosqueDetails(Mosques),
                CustomSpacerWidget(
                  width: 2,
                ),
                !mosqueAdmin ? Flexible(
                  child: status?_getUnFollowButton():_getFollowButton(),
                ):_getEditButton(),
                CustomSpacerWidget(
                  width: 28,
  ...
  _getEditButton() {
    return MosqueFollowButton(
      ...
      callBack: () {
        Navigator.of(context)
            .push(MaterialPageRoute(builder: (context) => CreateMosqueScreen(sessionUser:widget.sessionUser,isEdit: true,mosqueObject:adminMosques[0],)));
  ...
  _getMosqueDetails(List<Mosque> mosque) {
    return Padding(
      padding: const EdgeInsets.only(left: 28.0, top: 5.5, right: 10.0),
      child: Row(
        children: [
          _getMemberImage(mosque),
          CustomSpacerWidget(
            width: 10,
          ),
          _getNameAndLocation(mosque),
        ],
      ),
    );
  }

  _getMemberImage(List<Mosque> mosque) {
    return ClipRRect(
  ...
  _getNameAndLocation(List<Mosque> mosque) {
    return Padding(
  ...
  Future<List<Mosque>> getMosque(String mosqueID) async {
    try {
  ...
  Widget buildUI(List<Mosque> mosque,String userType) {
  ...
  Widget _adminMosqueCreateUI(List<Mosque> adminMosques) {
    return Scaffold(
      backgroundColor: AppColors.GREY_KIND,
      body: _getBody("admin"),
      bottomNavigationBar: _getBottomNavigation(),
  ...
  Future<List<Mosque>> getMosqueForAdmin() async {
    try {
      adminMosques = await Amplify.DataStore.query(Mosque.classType , where:Mosque.MOSQUE_ADMIN_ID.eq(widget.sessionUser.id));
      print("Before the details of mosque");
      print(adminMosques);
      print("inside the main page");
      return adminMosques;
