...
class _LoginScreenState extends State<LoginScreen> {
  ...
  _navigateUser(User loggedUser) async {
    ...
    if (loggedUser.email_verification == false) {
      ...
      //age check
    } else if (loggedUser.dob == null) {
      Navigation.intentWithData(
          context, AppRoutes.AGEREGISTER, PhoneOTPArgumentClass(loggedUser));

      // parent verification check
    } else if (loggedUser.parent_verification == false &&
        loggedUser.address_verification == false) {
      final dtToday = DateTime.now();
      final dtDate = DateTime.parse(loggedUser.dob.toString());
    ...
    //if all the verifications are done, then take user to Home screen
      ...
      context.read<UserDataProvider>().setUserData(loggedUser);
      Navigation.intentWithData(
